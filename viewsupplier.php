    ...
    <div class="profile-card">
        <div class="header">
            <div class="profileImage">
                <?php 
                    $dbs=mysqli_query($con,"SELECT Image from user where ID='$supplierid'");
                    $img=mysqli_fetch_assoc($dbs)['Image'];
                    if($img==NULL){
                        echo '<i class="fa-regular fa-circle-user"></i>';
                    }
            ...
            <div class="postsNumber">
                <?php 
                    $dbs=mysqli_query($con,"SELECT * from product where U_ID='$supplierid'");
            ...
            <div class="connectionsNumber">
                <?php 
                    $dbs=mysqli_query($con,"SELECT * from connection where uID='$supplierid' OR connectionID='$supplierid'");
                    echo "<div class=number>".mysqli_num_rows($dbs)."</div>";
                ?>
                <div>CONNECTIONS</div>
            </div>
            <div class="salesNumber">
                <?php 
                        $dbs=mysqli_query($con,"SELECT * from supplier where UID='$supplierid'");
        ...
        <div class="info">
            <?php 
                    $dbs=mysqli_query($con,"SELECT * from user where ID='$supplierid'");
        ...
        <div class="bio"><?php
                    $dbs=mysqli_query($con,"SELECT Bio from supplier where UID='$supplierid'");
        ...
        <div class="profileBtns">
            <button onclick="connect()" id="connectbtn"><?php 
                $dbconnect=mysqli_query($con,"SELECT * from connectionrequests where (uID='$userid' AND connectionID='$supplierid')OR(uID='$supplierid' AND connectionID='$userid')");
                if(mysqli_num_rows($dbconnect)!=0){
                    ...
                    $dbs=mysqli_query($con,"SELECT * from connection where (uID='$userid' AND connectionID='$supplierid')OR(uID='$supplierid' AND connectionID='$userid')");
            ...
            ?></button>
            <a href="mailto:<?php $dbs=mysqli_query($con,"SELECT * from user where ID='$supplierid'");
            $col=mysqli_fetch_array($dbs); echo $col['Email'];?>
            "><button>Contact</button></a>
            <a href="viewcatalog.php?sid=<?php echo $supplierid;?>"><button>View Catalog</button></a>
        </div>
        <div class="posts">
            <?php
    ...
    <script>
        ...
        function connect(){
            if(document.getElementById("connectbtn").innerHTML=="Connect"){
                ...
                var receiverId = "<?php echo $supplierid; ?>";
